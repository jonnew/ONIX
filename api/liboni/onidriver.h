...
// Note: The current spec specifies only one, but it might be better to leave the option open for future revisions
typedef enum {
...
ONI_DRIVER_EXPORT int oni_driver_init(oni_driver_ctx driver_ctx, int device_index);
...
// This gets called when oni_set_opt is called. This method does not need to perform any configuration
// But it is provided for the driver to do some internal adjustments if required
...
// Functions to get and set set driver-specific options. This kind of optiosn must be avoided when necessary to allow for a general interface
...
ONI_DRIVER_EXPORT const char* oni_driver_get_id();
