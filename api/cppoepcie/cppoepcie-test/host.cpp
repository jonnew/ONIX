...
void data_loop(std::shared_ptr<oe::context_t> ctx)
{
...
int main(int argc, char *argv[])
{
    ...
    for (const auto &d : dev_map) {
        ...
        const auto dev_str = oe::device_str(d.id);
    ...
    std::cout << "Acquisition clock rate: "
              << ctx->get_opt<uint32_t>(OE_ACQCLKHZ)
              << " Hz\n";

    // Start acquisition
    ctx->set_opt(OE_RUNNING, 1);
