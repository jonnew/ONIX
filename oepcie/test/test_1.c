...
int main(){
    ...
    printf("\tSignal input stream path set/get test...");
    char signal_check[100];
    size_t signal_len = 100;
    rc = oe_get_ctx_opt(ctx, OE_SIGNALSTREAMPATH, signal_check, &signal_len);
    assert(rc == 0);
    assert(!strcmp(signal_path, signal_check));
    assert(signal_len == strlen(signal_path) + 1);
    printf("pass.\n");

    // Write our desired info into the header file
    int config_id = 333;
    int device_one_id = RHD2032;
    int device_two_id = RHD2032;
    int device_three_id = MPU950;
    int neg_one = -1;
