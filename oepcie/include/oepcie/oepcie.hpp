...
template <size_t NumChannels, size_t BlockSamples>
class pcie_device {
...
public:
    ...
    // Raw data and buffer types
    using raw_data_t = int16_t;
    using raw_buffer_t
        = std::array<std::array<raw_data_t, BlockSamples>, NumChannels>;
                ...
                const char *ctrl_addr = "/dev/xillybus_control_regs_16")
    {
        ...
        if (data_fid_ < 0 || ctrl_fid_ < 0)
            throw std::runtime_error("Could not open data and/or control FIFOs. Are they in use?");
    ...
    int configure(config_addr ctrl_addr, uint16_t value, uint16_t mask = 0xffff)
    {
        ...
        if ((mask & 0xffff) != 0xffff) {
            ...
            if (rd < 2) {
                std::cerr << "Unsuccessful read to control address " << ctrl_addr
                          << " code: " << rd << std::endl;
                return -1;
            }
            if (lseek(ctrl_fid_, ctrl_addr, SEEK_SET) < 0) {
    ...
    int read_block(raw_buffer_t &block) // , int64_t &sample_number)
    {
