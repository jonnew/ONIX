...
//
//  MSB [ledlevel7, ledlevel6, ..., ledlevel0,
//       ledmode1, ledmod0,
...
// Signal       | Description
// -------------------------------------------------------------------------
// ledlevel     | 8 bit register for general LED brighness. 0 = off, 255 = brightest
// ledmode      | 2 bit register for LED mode. 0 = all off, 1 = only power/running, 2 = power/running, pll, harp, 3 = all on
// portx_status | 2 bit register describing the headstage port state. 0 = not_locked, 1 = locked, 2 = not_active, 3 = forced_off
...
always @ (posedge i_clk) begin
    ...
    // Shift in fast data
    shift_d0 <= {shift_d0[9:0], ddr_d0};

    // Sample slow clock to see if we should latch outputs
    if (i_clk_s == 'b1 && last_i_clk_s == 'b0) begin

        // NB: Data is not yet shifted right, so we are used 2 bit
        // left-shifted indices here

        // Update fast output port
        o_port <= {shift_d0[5:0], ddr_d0};
        ...
        case (shift_d0[9:8])
            'b00 : begin // Shift slow data in
                o_reset <= 'b0;
                //slow_valid <= 'b0;
            end
            'b01 : begin // Set output and reset shift register
                o_reset <= 'b0;
                //slow_valid <= 'b1;
                //slow_value <= slow_shift;

                o_ledlevel <= slow_shift[47:40];
...
SB_IO # (
    ...
    .INPUT_CLK(i_clk),
    .OUTPUT_ENABLE(1'b1),
    .D_IN_0(ddr_d0[1]), // rising
    .D_IN_1(ddr_d0[0])  // falling
);
endmodule
